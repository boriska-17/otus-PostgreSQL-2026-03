...
Подключимся к созданной БД из внешнего ресурса. В моём слечае - это установленный на ноутбуке pgAdmin длф Windows.
Создадим в нём новоу подключение по адресу 192.168.229.129 и выполним select из таблицы.
![](docker_select.png)

## Возникшие проблемы и их решение

При внешнем подключении pgAdmin не видел БД postgres по адресу localhost.
При помощи команды ip addr show на ВМ ubuntu посмотрел адреса и взял второй из списка:
![](ip_addresses.png)
